...
We will use the function _normalize_ for normalizing the data, and then the option _normcounts_ for accessing the normalized counts matrix:

```{r norm_matrix}

sc_exp_filter <- normalizeSCE(sc_exp_filter)
sc_exp_filter_norm_matrix <- normcounts(sc_exp_filter)
head(sc_exp_filter_norm_matrix, 10)
...
We will normalize the data using _NormalizeData_, and then access the  normalized matrix using _Object@data_.
